-- Hyväksyntä voi kirjoittaa YVA-vaihtoehtojen luvut lähteineen.
-- Vanha 6-parametrinen allekirjoitus korvataan; p_vaihtoehdot oletuksena tyhjä.

DROP FUNCTION IF EXISTS julkaise_ehdotetut_tiedot(text, uuid, jsonb, jsonb, uuid, text);

CREATE FUNCTION julkaise_ehdotetut_tiedot(
  p_tyyppi text,
  p_hanke_id uuid,
  p_hanke jsonb,
  p_lahteet jsonb,
...
COMMENT ON FUNCTION julkaise_ehdotetut_tiedot(
  text, uuid, jsonb, jsonb, uuid, text, jsonb
) IS
  'Julkaisee muutosehdotuksen hankekentat ja/tai hanke_vaihtoehdot-rivit lahdeineen.';
